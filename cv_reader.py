...
[SECTION 4] SKILLS & ADDITIONAL TRAINING
...
Return a list [SECTION 1A, SECTION 1B, SECTION 2, SECTION 3, SECTION 4]
...
SECTION 4 should be a list of skills and additional training [number of entires, entry1, entry2,…] where the first index represents how many entries there are and the following indices contain those additional skill and additional training bullet points. If there are none, make the first index 0.
...
Ensure University/Company Name, Location, Dates, Degree/Position, Dates, Description of Entry are 5 seperate entries in each of the sublists in sections 2 - education, 3 - work experience, and 4 - leadership experience. If nothing can be found, just leave the placeholder 'EMPTY' for that empty section, but make sure 'EMPTY' is there for indexing purposes.

Since the result will be used in code directly, please put any comments you have as a 6th "section" in the list object directly. Ensure that the size of the resultant list is only these 5 sections and the additional comments section [SECTION 1A, SECTION 1B, SECTION 2, SECTION 3, SECTION 4].

Make sure not to skip closing any lists. Make sure all the '[' and their respective ']' match up.

Make sure not to skip over any entries for work experience, skills, and education sections. If there are 5 seperate entries that you can comprehend related to work experience for instance, add all five in line with the aforemtioned formatting rules.
...
Lastly, remember, the number of sublists/entries in each section should be the first index in that respective list, not its own entry (as the indexing to access the information depends on this consistency). Likewise, pleasure leave no code comments in between the list as well and return NOTHING but the list. No additional text outside the list.
    ...
    [
        ...
        ["School", "Location", "Major/Course", "Dates", ["GPA", "Organizations", "Coursework"]]
    ],  
    ...
    # - Anything under internships and or volunteering will be facilitated under the Work Experience Section
    
]"""})
    # Sample CV Input
    conversation.append({"role": "user", "content": """Khamis Mohammad Hamad Almazrouei 
...
def docx_writer(formatted_cv_list):
    ...
    # List [SECTION 1A, SECTION 1B, SECTION 2, SECTION 3, SECTION 4]
    ...
    # SECTION 4 should be a list of skills and additional training where the first index represents how many entries there are and the following indices contain those additional skill and additional training bullet points. If there are none, make the first index 0.
    ...
    if formatted_cv_list[4] == 0:
        ...
        skills_sublists = formatted_cv_list[4][1:]
    ...
    section_2_header.add_run().add_break()

    # Add University and Location, Course and Dates Headers
    section_2_content = formatted_cv.add_paragraph()
    section_2_content.style = 'List Bullet'
    for sublist in education_section_sublists:
        if sublist[0] == "ERROR":
            ...
            run2_univ_loc = section_2_header.add_run(sublist[0] + " "*25 + sublist[1]) # 25 Spaces, will need to be Manually Fixed on Resultant Document
            section_2_header.add_run().add_break()
            run2_course_date = section_2_header.add_run(sublist[2] + " "*25 + sublist[3]) # 25 Spaces, will need to be Manually Fixed on Resultant Document
            section_2_header.add_run().add_break()

            # Add Description Points of Each Education Entry
            for point in sublist[4]:
                run2_desc = section_2_content.add_run(point)
                # section_2_content.add_run().add_break()
        # Univeristy and Location Formatting
        if run2_univ_loc is not None:
            run2_univ_loc.bold = True
            run2_univ_loc.font.size = Pt(11)
            run2_univ_loc.font.name = 'Times New Roman'

        # Course and Dates Formatting
        if run2_course_date is not None:
    ...
    section_3_header.add_run().add_break()

    # Add Company and Location, Position and Dates Headers
    section_3_content = formatted_cv.add_paragraph()
    section_3_content.style = 'List Bullet'
    for sublist in work_experience_sublists:
        if sublist[0] == "ERROR":
            ...
            run3_comp_loc = section_3_header.add_run(sublist[0] + " "*25 + sublist[1]) # 25 Spaces, will need to be Manually Fixed on Resultant Document
            section_3_header.add_run().add_break()
            run3_pos_date = section_3_header.add_run(sublist[2] + " "*25 + sublist[3]) # 25 Spaces, will need to be Manually Fixed on Resultant Document
            section_3_header.add_run().add_break()

            # Add Description Points of Each Work Experience Entry
            for point in sublist[4]:
                run3_desc = section_3_content.add_run(point)
                # section_3_content.add_run().add_break()
    
        # Company and Location Formatting
        if run3_comp_loc is not None:
            run3_comp_loc.bold = True
            run3_comp_loc.font.size = Pt(11)
            run3_comp_loc.font.name = 'Times New Roman'

        # Position and Dates Formatting
        if run3_pos_date is not None:
        ...
        # Content Formatting
        if run3_desc is not None:
            run3_desc.bold = False
            run3_desc.italic = False
            run3_desc.font.size = Pt(11)
            run3_desc.font.name = 'Times New Roman'

    # SECTION 4 - Add Skills and Additional Training Section
    # Add Header
    section_4_header = formatted_cv.add_paragraph()
    run4_header = section_4_header.add_run("Skills and Additional Training")
    run4_header.bold = True
    run4_header.font.size = Pt(12)
    run4_header.font.name = 'Times New Roman'

    # Add Content
    section_4_content = formatted_cv.add_paragraph()
    section_4_content.style = 'List Bullet'
    for entry in skills_sublists:
        if entry == "ERROR":
            run4_desc = section_4_content.add_run("This Section is Empty, Please Format it Accordingly!")  
        else:
            run4_desc = section_4_content.add_run(entry)
            section_4_content.add_run().add_break()

        # Content Formatting
        if run4_desc is not None:
